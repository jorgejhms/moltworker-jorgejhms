...
**Note:** The CLI tool is named `openclaw`, and internal config paths use the OpenClaw directory structure.
...
# Edit .dev.vars with your ZAI_API_KEY
npm run start
```
...
The Dockerfile includes a cache bust comment. When changing `openclaw.json.template` or `start-moltbot.sh`, bump the version:
